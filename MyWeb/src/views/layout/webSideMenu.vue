 ...
 */
const onClick = (data) => {
  if (data.url) $router.push(data.name)
  else ElNotification({ message: '无效菜单！', type: 'warning', 'show-close': false })
}
</script>

<template>
  <el-menu :default-active="activeIndex" class="el-menu-demo" mode="vertical">
    <el-sub-menu v-for="menu in menuData.data" :key="menu.name" :index="menu.name">
